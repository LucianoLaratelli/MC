...
void input(GCMC_System *sys) {
  char argon[] = "Ar", helium[] = "He", neon[] = "Ne", krypton[] = "Kr",
       xenon[] = "Xe", o2[] = "O2", water[] = "Water";
...
double distfinder(GCMC_System *sys, int id_a, int id_b) {
  double dist = 0.0, delta = 0.0, delta2 = 0.00;
...
double **matrix_madness(GCMC_System *sys) {
  ...
  for (int i = 0; i < n - 1; i++) {
    for (int j = i + 1; j < n; j++) {
      double r = distfinder(sys, i, j), rinv = 1 / r, rinv2 = rinv * rinv,
             rinv3 = rinv2 * rinv, rinv5 = rinv2 * rinv3,
...
double *pick_dipole_direction(GCMC_System *sys) {
  double theta = random_range(0, 2 * M_PI), z = random_range(-1, 1),
         x = sqrt(1 - (z * z)) * cos(theta), y = sqrt(1 - (z * z)) * cos(theta);
  double *dipole;
...
bool move_accepted(double cpe, double npe, MoveType move_type,
                   GCMC_System *sys) {
  double beta = 1.0 / (k * sys->system_temp); // thermodynamic beta
  double boltzmann_factor = pow(M_E, (-beta * (npe - cpe)));
  double acceptance;
  int pool = sys->particles.size(), poolplus = pool + 1;
  switch (move_type) {
  case TRANSLATE:
    acceptance = boltzmann_factor;
    break;
  case CREATE_PARTICLE:
    acceptance = boltzmann_factor * sys->volume * conv_factor /
                 (sys->system_temp * (double)pool);
    break;
  case DESTROY_PARTICLE:
    acceptance = boltzmann_factor * sys->system_temp * (double)poolplus /
                 (sys->volume * conv_factor);
    break;
  default:
    acceptance = -1.0;
  }
  return acceptance > random_range(0, 1);
}
...
void undo_insertion(GCMC_System *sys) { sys->particles.pop_back(); }

// undoes a rejected displacement
void unmove_particle(GCMC_System *sys) {
  int pick = sys->move.pick;
  double phi = sys->move.phi, gamma = sys->move.gamma, delta = sys->move.delta;
  sys->particles[pick].x[0] -= phi;
  sys->particles[pick].x[1] -= gamma;
  sys->particles[pick].x[2] -= delta;
...
void radialDistribution(GCMC_System *sys, int step) {
  ...
  int IK, n = sys->particles.size(), num_pairs = 0;
  double BinSize = sys->BinSize, expected_number_of_particles,
         diameter_of_current_sphere, diameter_of_previous_sphere, shell_volume,
         dist;
  for (int I = 0; I < n - 1; I++) {
    for (int K = I + 1; K < n; K++) {
  ...
  if (step == sys->maxStep - 1) {
    double real_density_hours =
        (sys->sumparticles / (sys->maxStep * .5)) / sys->volume;
  ...
  if (sys->output_flag) {
    if (sys->stockmayer_flag) {
      ...
      fprintf(sys->output,
              ...
              "0 %lf\n0 %lf\n0 %lf\n",
              sys->box_side_length, sys->box_side_length, sys->box_side_length);
