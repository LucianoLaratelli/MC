...
typedef struct particle_particle {
  double x[3], dipole_magnitude, dipole[3];
} particle;
...
typedef struct _GCMC_System {
  ...
  double epsilon, particle_mass, sigma, polarizability, dipole_magnitude,
      sigma_squared, // powers of sigma for PE
      sigma_sixth, sigma_twelfth;
  char particle_type[25];
  // system variables
  double system_temp, cutoff;
  double box_side_length;
  int maxStep, volume;
  // for averaging
  double sumparticles, sumenergy;
  // next three lines are for radial distribution function
  double BinSize = .5;
  int nBins, step;
  double *boxes;
  clock_t start_time;
  bool ideal_flag, energy_output_flag, stockmayer_flag, output_flag, debug_flag,
      NVT_flag;
} GCMC_System;
